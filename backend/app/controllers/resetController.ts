...
export const validatePasswordToken = async (req: CustomRequest, res: Response): Promise<void> => {
	try {
		...
		if (!dbToken || dbToken.is_used) {
			...
			res.status(400).json({ status: 404, message: 'Token not found or already used' });
			return;
		}
